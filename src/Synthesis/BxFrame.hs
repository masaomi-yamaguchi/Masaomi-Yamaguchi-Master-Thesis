...
makeFramesBAlts
  ...
  -- let exitCond = return $ MEHole (mTyArr v mBool) menv
  if isLen1
    then do
      ...
      -- let reconciler = return $ MEHole (mTyArr s (mTyArr v s)) menv
...
makeFramesExitCond isIdentical menv e p_e ty_v = do
  ...
  case p_e of
    Just p  -> do
      let alt1   -- DT.trace "debug: aaa"
            = if frame_default env || isIdentical
              then return (p, mTrue, mTrue)
                ...
                if b
                  then 
                       -- DT.trace "debug: not used!!"
                    return
                      ( p
                      , mTrue
                      , return $ MEHole False mBool (extendLocalM v ty_v env'))
                  else 
                         -- DT.trace "debug: used!!" $
                    return (p, mTrue, mTrue)
    ...
    Nothing -> return
      ...
      $ return
      $ MEHole False mBool (extendLocalM v ty_v menv)
...
makeFramesP :: Int -> TExp -> Maybe P.Pat
makeFramesP loc e = let (ans, _) = go loc e
                    in case ans of
                         (P.PVar _) -> Nothing
                         _          -> Just ans
  where
    ...
    case p_maybe of
      Just p  -> do
        ...
        let alt1 =
              ...
              then return (p, mTrue, return (MEHole False ty_s env'))
                <|> return (p, mTrue, destructPR env' (dataEnv env) p0 ty_s)
              else return (p, mTrue, return (MEHole False ty_s env'))
        let mcase = return
      ...
      Nothing -> do
        ...
        let e' = if frame_default env
                 then return (MEHole False ty_s env')
                   <|> destructPR env' (dataEnv env) p0 ty_s
                 else return (MEHole False ty_s env')
        return $ MEAbs False (return s) $ return $ MEAbs False (return v) e'
  where
          -- $ return
          -- $ MEHole False ty_s env'
...
checkIdenticalMaybe :: Maybe P.Pat -> Maybe P.Pat -> Bool
checkIdenticalMaybe (Just p1) (Just p2) = checkIdentical p1 p2
