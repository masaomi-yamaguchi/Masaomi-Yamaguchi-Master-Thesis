...
evalU env (MEEq _ e1 e2) = do
  ...
  where
    check :: MonadFail m => MRExp m -> MRExp m -> m Bool
    ...
    checkL
      :: MonadFail m => m (List m (MRExp m)) -> m (List m (MRExp m)) -> m Bool
    checkL ml1 ml2 = do
      l1 <- ml1
      l2 <- ml2
      case (l1, l2) of
...
-- evalU (MELift e1 e2) _ _ = undefined
-- evalU (MEHole _ _) _ _ = undefined
-- r1, r2どちらもshareされていることを前提にしている
appR
