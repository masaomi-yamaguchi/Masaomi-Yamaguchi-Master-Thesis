...
data Example = Example Value Value Value
...
checkGetPut examples e env = mapM_ (\example -> go example e env) examples
  where
    go :: (MonadPlus m, Sharing m, MonadFail m, Delay m)
       ...
       -> m (MREnv m)
       -> m ()
    go (Example s v s') e env = do
      (v_, trace) <- LG.getTrace e s' env
      eqv <- equalVM v v_
      if eqv
        then do
          trace <- share trace
          -- LPN.putTraceCheck env e s v s' trace
...
fillHole with_flag pen nonBxEnv me = do
  ...
  case e of
    ...
    MEAbs _ x e1       -> return
      (MEAbs False x (fillHole with_flag pen nonBxEnv e1))
    ...
    MEEq _ e1 e2       -> return
      (MEEq
         ...
         (fillHole with_flag pen nonBxEnv e2))
    MENeg _ e1         -> return
      (MENeg False (fillHole with_flag pen nonBxEnv e1))
